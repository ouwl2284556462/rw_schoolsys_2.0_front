		...
		<template v-slot:main>
			<form id="user-info-dtl-from" class="form-horizontal"  @submit.prevent="onSubmit">
				...
				<div class="form-group">
					...
					<div class="col-xs-8 col-sm-6">
						<cselect class="form-control" id="sex" name="sex" :listItem="sexEnum" :isRequire="true" :val="userInfo.sex">
						</cselect>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-xs-8 col-sm-6">
						<cselect class="form-control" id="identityType" name="identityType" :listItem="identityEnum" :isRequire="true"
				...
				<div class="form-group" v-if='isAdminAddOrChgUser'>
					...
					<div class="col-xs-8 col-sm-6">
						<cselect class="form-control" id="role" name="role.name" :listItem="roleEnum" :isRequire="true" :val="userInfo.role.name">
						</cselect>
					</div>
				</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			initSelectEnum() {
				let qryParam = {};
				qryParam.groupIds = ['sex', 'identity_type', 'sys_role'];

				this.$serverApi.get('/sysDict/getSysItemsBatch.do', qryParam)
					.then(res => {
						if (NetConst.RESP_SUCCESS !== res.code) {
							this.$noticeWin.showMsg("获取参数失败");
							return;
						}

						let data = res.data;
						this.sexEnum = data.sex;
						this.identityEnum = data.identity_type;
						this.roleEnum = data.sys_role;
					}).catch(err => {
						this.$noticeWin.showMsg("获取参数失败");
					});
			},
