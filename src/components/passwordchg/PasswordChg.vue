<template>
	<main-panel title="修改密码">
		<form id="password-chg-from" class="form-horizontal" th:data-admin-userpasswordchg-url="@{/UserManage/changePassword.do}"
		 @submit.prevent="onFormSubmit">
			...
			<div class="form-group text-center bottom-btn-area">
				<cbtn id="password-chg-form-submitBtn" type="submit" class="btn btn-primary" :showLoading='isDealing'>修改</cbtn>
				<cbtn v-if='isAdminChgUser' id="password-chg-form-returnBtn" class="btn btn-primary">返回</cbtn>
			</div>
		</form>
	</main-panel>
</template>
	...
	export default {
		...
		methods: {
			...
			onFormSubmit(event) {
				...
				this.isDealing = true;
				this.$serverApi.post("/Workspace/updateCurUserPassword.do", formData).then(res => {
